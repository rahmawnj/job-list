...
class JobController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax() && $request->has('draw')) {
            ...
            $query = Job::query()->with(['company', 'jobcategory'])->withCount('applyjobs');
            if ($search !== '') {
                $query->where(function ($q) use ($search) {
                    $q->where('title', 'like', "%{$search}%")
                      ...
                      ->orWhereHas('jobcategory', fn ($qc) => $qc->where('name', 'like', "%{$search}%"));
                });
            }
            ...
            if ($order && isset($order['column'], $order['dir'])) {
                ...
                switch ($orderColumn) {
                    case 'title': $query->orderBy('jobs.title', $orderDir); break;
                    case 'company_name': $query->orderBy(Company::select('name')->whereColumn('companies.id', 'jobs.company_id'), $orderDir); break;
                    case 'jobcategory_name': $query->orderBy(Jobcategory::select('name')->whereColumn('jobcategories.id', 'jobs.jobcategory_id'), $orderDir); break;
                    case 'apply_count': $query->orderBy('applyjobs_count', $orderDir); break;
                    default: $query->orderBy('jobs.id', $orderDir);
                }
            } else { $query->orderBy('jobs.id', 'desc'); }
            $jobs = $query->skip($start)->take($length)->get();
            $data = $jobs->map(function ($job, $key) use ($start) {
                return [
                    'no' => $start + $key + 1,
                    'image' => $job->image ? asset('storage/' . $job->image) : asset('img/default.png'),
                    'title' => $job->title,
    ...
    {
        return view('dashboard.jobs.create', ['title' => 'Jobs', 'companies' => Company::all(), 'locations' => Location::all(), 'jobcategories' => Jobcategory::all()]);
    }

    public function store(Request $request)
    {
        $validatedData = $request->validate(['company_id' => 'required', 'jobcategory_id' => 'required', 'location_id' => 'required', 'type' => 'required', 'title' => 'required', 'status' => 'required', 'image' => 'image|file|max:2048']);
        ...
        if ($request->file('image')) $validatedData['image'] = $request->file('image')->store('uploads/image/jobs');
    ...
    {
        return view('dashboard.jobs.show', ['title' => 'Jobs', 'job' => $job]);
    }

    public function edit(Job $job)
    {
        return view('dashboard.jobs.edit', ['title' => 'Jobs', 'job' => $job, 'companies' => Company::all(), 'locations' => Location::all(), 'jobcategories' => Jobcategory::all()]);
    }

    public function update(Request $request, Job $job)
    {
        $request->validate(['company_id' => 'required', 'jobcategory_id' => 'required', 'location_id' => 'required', 'title' => 'required', 'type' => 'required', 'status' => 'required', 'logo' => 'image|file|max:2048']);
        if ($request->file('image')) {
            if ($job->image) Storage::delete([$job->image]);
    ...
    {
        return view('dashboard.jobs.dump', ['title' => 'Jobs Dump', 'jobs' => Job::onlyTrashed()->get()]);
    }
    ...
    public function apply_jobs(Job $job)
    {
        ...
        return view('dashboard.jobs.apply', ['title' => 'Jobs', 'job' => $job, 'applyjobs' => $applyjobs]);
    }
    ...
    public function apply_job_detail($apply_job_id)
    {
        ...
        return view('dashboard.jobs.apply_detail', ['apply_job' => $apply_job, 'title' => 'Jobs']);
    }

    public function update_apply_job_detail(Request $request, ApplyJob $apply_job)
    {
        $request->validate(['status' => 'required|in:pending,approved,rejected']);
        $apply_job->update(['note' => $request->note, 'status' => $request->status]);
    ...
    public function exportCandidates(Job $job)
    {
        ...
        if ($jobCandidates->isEmpty()) {
            $rows = '<tr><td colspan="14">No candidate assigned.</td></tr>';
        } else {
            foreach ($jobCandidates as $jobCandidate) {
                $milestones = $jobCandidate->milestones;
                if ($milestones->isEmpty()) $milestones = collect([null]);
        ...
        $html = '<html><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif}table{border-collapse:collapse}th,td{border:1px solid #ccc;padding:7px;vertical-align:top}th{font-weight:bold;background:#f2f2f2;white-space:nowrap}</style></head><body>'
            . '<h2>Recruitment Process Report</h2>'
            . '<table><tr><th>Candidate Name</th><th>Email</th><th>Phone</th><th>CV Link</th><th>Company</th><th>Position / Jabatan</th><th>Job Category</th><th>Job Type</th><th>Salary / Gaji</th><th>Total Position</th><th>Location</th><th>Job Status</th><th>Recruitment Step</th><th>Step Status</th><th>Step Date</th><th>Notes</th><th>Related Link</th></tr>'
    ...
    {
        $validated = $request->validate(['candidate_id' => 'required|exists:candidates,id']);
        $alreadyAssigned = JobCandidate::where('job_id', $job->id)->where('candidate_id', $validated['candidate_id'])->exists();
        if ($alreadyAssigned) return redirect()->back()->withErrors(['candidate_id' => 'Candidate sudah di-assign ke job ini.'])->withInput();
        JobCandidate::create(['job_id' => $job->id, 'candidate_id' => $validated['candidate_id']]);
        return redirect()->route('admin.job.candidates', $job->id)->with('success', 'Candidate berhasil di-assign.');
    }

    public function updateCandidate(Request $request, Job $job, JobCandidate $jobCandidate)
    {
        if ($jobCandidate->job_id !== $job->id) abort(404);
        $validated = $request->validate(['candidate_id' => 'required|exists:candidates,id']);
        $alreadyAssigned = JobCandidate::where('job_id', $job->id)->where('candidate_id', $validated['candidate_id'])->where('id', '!=', $jobCandidate->id)->exists();
        if ($alreadyAssigned) return redirect()->back()->withErrors(['candidate_id' => 'Candidate sudah di-assign ke job ini.'])->withInput();
        $jobCandidate->update(['candidate_id' => $validated['candidate_id']]);
        return redirect()->route('admin.job.candidates', $job->id)->with('success', 'Assignment candidate berhasil diperbarui.');
    }
